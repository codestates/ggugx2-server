export const USER_ALREADY_EXISTS = 'user already exists!';
export const PHONENUMBER_OR_PASSWORD_MISSING =
  'phoneNumber or password is missing!';
export const WRONG_PASSWORD = 'wrong password!';
export const USER_NOT_EXISTS = 'user not exists!';
export const STORE_ALREADY_EXISTS = 'store already exists!';
export const STORE_NOT_EXISTS = 'store not exists!';
export const BLANKS_MISSING = 'Please fill out all the blanks!';
export const COUPON_NOT_EXISTS = 'Sorry no coupon!';
export const REWARD_NOT_EXISTS = 'Sorry no reward!';
export const NO_SUCH_CUSTOMER_OR_STORE = 'no such customer or store!';
export const USERNAME_IS_NULL =
  'username is missing please checkout the field again!';
export const PASSWORD_IS_NULL = 'passowrd is missing or incorrect, try again!';
